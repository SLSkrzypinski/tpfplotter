...
# Now it is callable as a function
...
def tpfplotter(tic,LIST=False,COORD=False,SAVEGAIA=False,name=False,maglim=5,sector=None,gid=None,gmag=None,legend='best'):
    ...
    for tt,tic in enumerate(tics):
        ...
        if COORD is not False:                                                                                          #
            plt.title('Coordinates '+tic+' - Sector '+str(tpf.sector), fontsize=16, zorder=200,color='white')# + ' - Camera '+str(tpf.camera))  #
        elif name is not False:
            plt.title(name +' - Sector '+str(tpf.sector), fontsize=15, zorder=200,color='white')
        else:   												#
        ...
        cb.set_label(r'Flux '+exponent+r' (e$^-$)', labelpad=10, fontsize=16,color='white')
